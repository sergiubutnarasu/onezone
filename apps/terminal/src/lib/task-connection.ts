...
import type { ActiveProcessEntry, TaskConnectionDeps } from "./types/index.js";
...
export function connectToTask(deps: TaskConnectionDeps): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const {
      ...
      isClosed: isSocketClosed,
    } = createTaskSocket(serverUrl, taskId, terminalId, terminalName, {
      onConnect: () => {
        const deps = {
          socket,
          roomId,
        ...
        if (activeProcesses.size > 0) {
          log(
            `[${terminalName}] [${roomId}] Reconnected — process still running, resuming`,
          );
          return;
        }
      ...
      onMessage: (event: string, payload: unknown) => {
        ...
        switch (event) {
          case EventCommands.TaskDeleted: {
            log(`[${terminalName}] [${roomId}] Task deleted, disconnecting...`);
          ...
          case EventCommands.TerminalCommandPing: {
            ...
            if (entry?.writeStdin) {
              log(
                `[${terminalName}] [${roomId}] Pinging job ${jobId} with input: ${input}`,
              );
              entry.writeStdin(input + "\n");
            } else {
              log(
                `[${terminalName}] [${roomId}] Ping ignored — no active job ${jobId} (found=${!!entry}, hasWriteStdin=${!!entry?.writeStdin})`,
              );
            }
            break;
          }
          ...
          case EventCommands.TerminalCommandRun: {
            ...
            spawnCommand({
              ...
              deps.log(
                `[${terminalName}] [${roomId}] spawnCommand error: ${(err as Error).message}`,
              ),
            );
            break;
          }

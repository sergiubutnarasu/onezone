...
function buildPrompt(payload: ProjectBuilderCommandPayload, serverUrl: string): string {
  ...
  return [
    ...
    "Mandatory: every installed skill must be referenced by name in the instructions of the columns that use it, per the skill's mandatory skill-usage rule. Do not install a skill that no column references.",

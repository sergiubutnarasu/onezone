...
5. Your very first action in every session must be to read `config/memories/project-memory.md` (use `cat config/memories/project-memory.md` or equivalent). Do this before anything else, even if the task seems simple. If the file does not exist, proceed without it. Never skip this step.
...
7. For any code search or exploration - finding where something is implemented, locating symbols, understanding how a feature works - first check if semble is available with `semble -h`. If it is, use it instead of grep, glob, or reading files speculatively. Examples:
   ...
   If `semble -h` fails, fall back to grep/glob as normal.
